...
module DifferenceDFA = struct
	...
	let invert_dfa (dfa : 'a DFA.dfa) : 'a DFA.dfa = let (states, alphabet, delta, s, accepts, comparator) = DFA.deconstruct_dfa dfa in
		DFA.build_dfa states alphabet delta s (cept states accepts comparator) comparator
	...
	let delta_use (delta : ('a * char * 'a) list) (state : 'a) (input : char) (comparator : 'a -> 'a -> bool) : 'a = 
		let resulting_state = 
		List.fold_left (fun acc (p,s,q) -> 
			if comparator p state && compare s input == 0 then Some q else acc) 
		None delta in
	...
	let visit_state (states_tracker : ('a * bool) list) (state : 'a) (comparator : 'a -> 'a -> bool) : ('a * bool) list = 
		List.fold_left (fun acc (state',visited) -> 
			if comparator state state' then (state',true)::acc else (state',visited)::acc) 
		[] states_tracker

	(* check if a state has been visited *)
	let state_visited (states_tracker : ('a * bool) list) (state : 'a) (comparator : 'a -> 'a -> bool) : bool = 
		List.fold_left (fun acc (state',visited) -> 
			if comparator state state' then visited else acc) 
		false states_tracker
	...
	* requires that the two dfa share a comparison function *)
	let find_difference_in_dfas (dfa1 : 'a DFA.dfa) (dfa2 : 'a DFA.dfa) : char list =
		...
		else let (states1, alphabet1, delta1, s1, accepts1, comparator1) = DFA.deconstruct_dfa dfa1 and
				 (states2, alphabet2, delta2, s2, accepts2, comparator2) = DFA.deconstruct_dfa dfa2 in
			if not (Shared.lists_equal alphabet1 alphabet2 Shared.comparator_of_compare) then raise AlphabetsNotEqual 
