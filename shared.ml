...
module Shared = struct
	...
	let lists_equal (lst1 : 'a list) (lst2 : 'a list) (comparator : 'a -> 'a -> bool) : bool = 
		(List.fold_left (fun acc el1 -> 
			if List.exists (fun el2 -> if comparator el1 el2 then true else false) lst2 then acc else false) 
		true lst1) &&
		(List.fold_left (fun acc el2 -> 
			if List.exists (fun el1 -> if comparator el1 el2 then true else false) lst1 then acc else false) 
		true lst2)
	...
	let rec closure (states : 'a list) (delta : ('a * char * 'a) list) (comparator : 'a -> 'a -> bool) : ('a list) = 
		let states' = List.fold_left (fun acc state -> 
				...
				acc delta) 
			states states 
		in let states'' = remove_dups states' comparator in 
	...
	let intersect (lst1 : 'a list) (lst2 : 'a list) (comparator : 'a -> 'a -> bool) : 'a list =
		List.fold_left (fun acc el1 -> 
			if List.fold_left (fun found el2 -> if comparator el1 el2 then true else found) false lst2 then el1::acc else acc)
		[] lst1
	...
	let delta_fresh_vars (delta : (string * char * string) list) (assoc_list : (string * string) list) : (string * char * string) list  = 
		(List.fold_left (fun acc (p,a,q) -> 
			(List.assoc p assoc_list,a, List.assoc q assoc_list)::acc) 
		[] delta)
