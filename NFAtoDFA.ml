...
module NFAtoDFA = struct
	...
	let delta_relations_equal (pstates1,char1,qstates1) (pstates2,char2,qstates2) (comparator : 'a -> 'a -> bool) : bool = 
		...
		| h::t -> let (accept_states : 'a list list) = 
			List.fold_left (fun dfa_accept_states dfa_state -> 
				List.fold_left (fun dfa_accept_states state -> 
					List.fold_left (fun dfa_accept_states accept_state -> if comparator state accept_state
