...
module NFAtoDFA = struct
	...
	let add_state (old_states : 'a list list) (new_state : 'a list) (comparator : 'a -> 'a -> bool) : 'a list list = 
		let repeat = 
		List.fold_left (fun acc old_state ->
			if (lists_equal new_state old_state comparator) then true else acc) 
		false old_states in 
		if repeat then old_states else new_state::old_states
	...
	: ('a list list) * ((('a list) * char * ('a list)) list) = 
		...
		if (List.length states == List.length new_states) && (List.length delta == List.length new_delta) then (new_states,new_delta) 
		else convert_to_DFA nfa new_states new_delta
